...
IMAGES = {
    ...
    'history': 'https://s10.iimage.su/s/05/gIKluIYxLgUpL28XWRh1IzTmdY4DzqMQuX8v39ee9.jpg'
}
...
SHOP_ITEMS = {
    ...
    'turbo': {
        'name': 'Турбо-день',
        'description': 'Двойное изменение цен (риск/доход)',
        'price': 400,
        'emoji': '⚡',
        'effect': 'turbo',
        'category': 'consumable'
    ...
    'magnet': {
        'name': 'Магнит',
        'description': 'Притягивает удачную сделку',
        'price': 350,
        'emoji': '🧲',
        'effect': 'magnet',
        'category': 'consumable'
    ...
    'investor': {
        'name': 'Инвестор',
        'description': '+20% к следующей покупке',
        'price': 600,
        'emoji': '💰',
        'effect': 'investor',
        'category': 'consumable'
    ...
    'fortune': {
        'name': 'Фортуна',
        'description': 'Удваивает случайную сделку',
        'price': 450,
        'emoji': '🎯',
        'effect': 'fortune',
        'category': 'consumable'
    ...
    'reinvest': {
        'name': 'Реинвест',
        'description': 'Возврат 10% от сделки',
        'price': 550,
        'emoji': '🔄',
        'effect': 'reinvest',
        'category': 'consumable'
    ...
    'shield2': {
        'name': 'Щит II',
        'description': 'Защита от 3 убыточных сделок',
        'price': 800,
        'emoji': '🛡️',
        'effect': 'shield2',
        'category': 'consumable'
    ...
    'analyst2': {
        'name': 'Аналитик II',
        'description': 'Точный прогноз цены',
        'price': 600,
        'emoji': '📊',
        'effect': 'analyst2',
        'category': 'consumable'
    ...
    'vip_month': {
        'name': 'VIP на месяц',
        'description': '30 дней: +10% к доходу, эксклюзивные предметы',
        'price': 5000,
        'emoji': '💎',
        'effect': 'vip',
    ...
    'trader_month': {
        'name': 'Трейдер PRO',
        'description': '30 дней: +15% к доходу, аналитика, страховка',
        'price': 8000,
        'emoji': '📈',
        'effect': 'trader_pro',
        'category': 'subscription',
    ...
    'investor_month': {
        'name': 'Инвестор',
        'description': '30 дней: +20% к доходу, приоритетная поддержка',
        'price': 12000,
        'emoji': '💰',
        'effect': 'investor_pro',
        'category': 'subscription',
    ...
    'lucky_box': {
        'name': 'Лаки бокс',
        'description': 'Случайный предмет (может выпасть что угодно)',
        'price': 300,
        'emoji': '🎁',
        'effect': 'lucky_box',
        'category': 'special'
    ...
    'mystery_box': {
        'name': 'Мистик бокс',
        'description': 'Редкие предметы, шанс на подписку',
        'price': 1000,
        'emoji': '📦',
        'effect': 'mystery_box',
        'category': 'special'
    ...
    # Сезонные
    'spring_booster': {
        'name': 'Весенний ускоритель',
        'description': 'Усиленный ускоритель (+75%)',
        'price': 800,
        'emoji': '🌸',
        'effect': 'spring_booster',
        'category': 'seasonal',
        'season': 'ВЕСНА'
    }
}
...
def buy_stock(user_id, ticker, amount):
    ...
    # Применяем бонус инвестора
    if user['effects'].get('investor'):
        total = total * 0.8  # -20%
        user['effects']['investor'] = False
    ...
    if user['money'] >= total:
        ...
        # Применяем бонус реинвеста
        if user['effects'].get('reinvest'):
            user['money'] += total * 0.1
            user['effects']['reinvest'] = False
            save_user(user_id, user)
...
def sell_stock(user_id, ticker, amount):
    ...
    # Применяем фортуну
    if user['effects'].get('fortune') and random.random() < 0.5:
        total *= 2
        user['effects']['fortune'] = False
...
def activate_item(user_id, item_index):
    ...
    if effect == 'booster':
        ...
        user['effects']['booster_until'] = str(datetime.now() + timedelta(days=1))
        msg = "🚀 Ускоритель активирован! Цены будут расти на 50% быстрее 24 часа"
    
    elif effect == 'shield':
        user['effects']['shield'] = user['effects'].get('shield', 0) + 1
        msg = "🛡️ Страховка активирована! Защитит от 1 убытка"
    
    elif effect == 'shield2':
        user['effects']['shield'] = user['effects'].get('shield', 0) + 3
        msg = "🛡️ Щит II активирован! Защитит от 3 убытков"
    
    elif effect == 'analyst':
        companies = load_companies()
        ticker = random.choice(list(companies.keys()))
        direction = random.choice(['вырастет', 'упадёт'])
        percent = random.randint(1, 10)
        msg = f"📊 Аналитик: {ticker} скорее всего {direction} на {percent}%"
    
    elif effect == 'analyst2':
        companies = load_companies()
        ticker = random.choice(list(companies.keys()))
        change = random.uniform(-0.15, 0.15)
        msg = f"📊 Аналитик II: {ticker} изменится на {change:+.1%}"
        ...
        msg = "💎 VIP статус активирован! +10% к доходу на 30 дней"
    
    elif effect == 'trader_pro':
        user['effects']['trader_pro'] = True
        user['subscriptions'].append({
            'type': 'trader_pro',
            'expires': str(datetime.now() + timedelta(days=30))
        })
        msg = "📈 Трейдер PRO активирован! +15% к доходу на 30 дней"
    
    elif effect == 'investor_pro':
        user['effects']['investor_pro'] = True
        user['subscriptions'].append({
            'type': 'investor_pro',
            'expires': str(datetime.now() + timedelta(days=30))
        })
        msg = "💰 Инвестор активирован! +20% к доходу на 30 дней"
    
    elif effect == 'predictor':
        trend = random.choice(['растущий', 'падающий', 'боковой'])
        msg = f"🔮 Предсказатель: следующие 3 дня рынок будет {trend}"
    
    elif effect == 'turbo':
        user['effects']['turbo'] = True
        msg = "⚡ Турбо-день активирован! Цены изменятся вдвое сильнее"
    
    elif effect == 'magnet':
        user['effects']['magnet'] = True
        msg = "🧲 Магнит активирован! Следующая сделка будет удачной"
    
    elif effect == 'investor':
        user['effects']['investor'] = True
        msg = "💰 Инвестор активирован! Следующая покупка -20%"
    
    elif effect == 'fortune':
        user['effects']['fortune'] = True
        msg = "🎯 Фортуна активирована! Шанс удвоить сделку"
    
    elif effect == 'reinvest':
        user['effects']['reinvest'] = True
        msg = "🔄 Реинвест активирован! Возврат 10% от сделки"
    
    elif effect == 'lucky_box':
        # Случайный предмет из обычных
        items = [k for k, v in SHOP_ITEMS.items() if v['category'] == 'consumable']
        if items:
        ...
        # Редкий предмет или подписка
        if random.random() < 0.3:  # 30% шанс на подписку
            subs = [k for k, v in SHOP_ITEMS.items() if v['category'] == 'subscription']
            if subs:
                random_sub = random.choice(subs)
                user['inventory'].append({
                    'item_id': random_sub,
                    'name': SHOP_ITEMS[random_sub]['name'],
                    'emoji': SHOP_ITEMS[random_sub]['emoji'],
                    'description': SHOP_ITEMS[random_sub]['description'],
                    'effect': SHOP_ITEMS[random_sub].get('effect', ''),
                    'bought_at': str(datetime.now())
                })
                msg = f"📦 Тебе выпала подписка: {SHOP_ITEMS[random_sub]['emoji']} {SHOP_ITEMS[random_sub]['name']}!"
        else:
            items = [k for k, v in SHOP_ITEMS.items() if v['category'] == 'consumable']
            if items:
                random_item = random.choice(items)
    ...
    elif effect == 'spring_booster':
        user['effects']['spring_booster'] = True
        user['effects']['booster_until'] = str(datetime.now() + timedelta(days=1))
        msg = "🌸 Весенний ускоритель активирован! +75% к росту цен на 24 часа"
...
def get_shop_main_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("📦 Расходники", callback_data="shop_consumable"),
        types.InlineKeyboardButton("💎 Подписки", callback_data="shop_subscription"),
        types.InlineKeyboardButton("🎁 Спецпредложения", callback_data="shop_special"),
        types.InlineKeyboardButton("🏆 Сезонные", callback_data="shop_seasonal"),
...
def get_shop_category_keyboard(category):
    ...
    for i in range(0, len(items), 2):
        ...
        row.append(types.InlineKeyboardButton(
            f"{items[i][1]['emoji']} {items[i][1]['name']} ${items[i][1]['price']}",
            callback_data=f"shop_item_{items[i][0]}"
        ))
        if i+1 < len(items):
            row.append(types.InlineKeyboardButton(
                f"{items[i+1][1]['emoji']} {items[i+1][1]['name']} ${items[i+1][1]['price']}",
                callback_data=f"shop_item_{items[i+1][0]}"
            ))
        markup.add(*row)
    
    markup.add(types.InlineKeyboardButton("◀️ Назад в магазин", callback_data="back_to_shop"))
    return markup
...
def get_inventory_keyboard(user):
    ...
    for i in range(0, len(items), 2):
        row = []
        row.append(types.InlineKeyboardButton(
            f"{items[i]['emoji']} {items[i]['name']}",
            callback_data=f"inv_item_{i}"
        ))
        if i+1 < len(items):
            row.append(types.InlineKeyboardButton(
                f"{items[i+1]['emoji']} {items[i+1]['name']}",
                callback_data=f"inv_item_{i+1}"
            ))
        markup.add(*row)
...
def get_item_action_keyboard(item_id, from_shop=True):
    markup = types.InlineKeyboardMarkup(row_width=2)
    if from_shop:
        markup.add(
        ...
        markup.add(
            types.InlineKeyboardButton("✅ Активировать", callback_data=f"activate_item_{item_id}"),
            types.InlineKeyboardButton("◀️ Назад", callback_data="back_to_inventory")
        )
    return markup
...
def start_command(message):
    ...
    story = (
        "📜 *История Terminal Trade*\n\n"
        "Ты — обычный человек, который однажды утром открыл Telegram и наткнулся на странный бот. "
        "В описании было сказано: _«Хочешь стать миллионером? Начни с $1000 и покори биржу!»_.\n\n"
        "Сначала ты подумал, что это очередной развод, но любопытство взяло верх. "
        "Ты нажал /trade и... оказался в мире, где каждое твоё решение может принести состояние или оставить с пустым кошельком.\n\n"
        "Теперь ты здесь. Ты — трейдер. И от твоих решений зависит, войдёшь ли ты в историю как легенда или исчезнешь в бездне убытков.\n\n"
        "Добро пожаловать в *Terminal Trade*. Не подведи."
    )
...
def help_command(message):
    ...
    help_text = (
        ...
        "• /tsell TSLA 2 — продать 2 акции Tesla\n"
        ...
        "*Сезоны:*\n"
        "Каждый сезон длится 14 дней. Топ-3 получают уникальные кубки, которые хранятся в инвентаре.\n\n"
        
        "*Совет:* не держи все деньги в одном месте — диверсифицируй портфель!"
    )
...
def list_command(message):
    ...
    if IMAGES.get('companies'):
        ...
        bot.send_message(message.chat.id, text, parse_mode="Markdown", reply_markup=get_companies_keyboard(1, total_pages))
...
def inventory_command(message):
    ...
    if IMAGES.get('inventory'):
        ...
        bot.send_message(message.chat.id, text, parse_mode="Markdown", reply_markup=get_inventory_keyboard(user))

@bot.message_handler(commands=['shop'])
def shop_command(message):
    text = "🛒 *МАГАЗИН TERMINAL TRADE*\n\nВыбери категорию товаров:"
    ...
    if IMAGES.get('shop'):
        ...
        bot.send_message(message.chat.id, text, parse_mode="Markdown", reply_markup=get_shop_main_keyboard())
...
Используй /help для списка команд
    ...
    elif call.data == "back_to_inventory":
        user = get_user(user_id)
        ...
        if call.data == "shop_consumable":
            category = "consumable"
            title = "📦 *Расходники*"
        elif call.data == "shop_subscription":
            category = "subscription"
            title = "💎 *Подписки*"
        elif call.data == "shop_special":
            category = "special"
            title = "🎁 *Спецпредложения*"
        elif call.data == "shop_seasonal":
            category = "seasonal"
            title = "🏆 *Сезонные*"
        else:
            bot.answer_callback_query(call.id)
        ...
        if IMAGES.get('shop'):
            bot.edit_message_media(
                ...
                call.message.chat.id,
                call.message.message_id,
                reply_markup=get_shop_category_keyboard(category)
            )
        else:
            bot.edit_message_text(
                text,
...
📋 *Портфель:*
        ...
        if IMAGES.get('main'):
            bot.edit_message_media(
                types.InputMediaPhoto(IMAGES['main'], caption="⏩ Новый день! Цены обновлены.", parse_mode="Markdown"),
                call.message.chat.id,
                call.message.message_id,
                reply_markup=get_back_keyboard("main")
            )
    ...
    elif call.data.startswith("inv_item_"):
        item_index = int(call.data.replace("inv_item_", ""))
        user = get_user(user_id)
...
_Нажми «Активировать», чтобы использовать предмет_
        """
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            parse_mode="Markdown",
            reply_markup=get_item_action_keyboard(item['item_id'], from_shop=False)
        )
        bot.answer_callback_query(call.id)
    
    elif call.data.startswith("activate_item_"):
        item_id = call.data.replace("activate_item_", "")
        
        # Находим предмет в инвентаре по item_id
        user = get_user(user_id)
        item_index = -1
...
💳 Твой баланс: ${user['money']:,.2f}
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            parse_mode="Markdown",
            reply_markup=get_item_action_keyboard(item_id, from_shop=True)
        )
        bot.answer_callback_query(call.id)
...
/adminuserinfo ID — информация об игроке
/adminuserlist — список всех игроков

*Управление предметами:*
/adminadditem ID ITEM_ID — выдать предмет
/adminremoveitem ID ITEM_ID — забрать предмет
...
*Рассылки:*
/adminbroadcast ТЕКСТ — отправить всем
/adminbroadcastactive ТЕКСТ — отправить активным сегодня
...
@bot.message_handler(commands=['adminuserinfo'])
def admin_user_info(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) < 2:
            bot.reply_to(message, "❌ Использование: /adminuserinfo ID")
            return
            
        target_id = int(parts[1])
        
        user = get_user(target_id)
        capital = calculate_capital(target_id)
        
        text = f"""
👤 *Информация об игроке {target_id}*

💰 Баланс: ${user['money']:,.2f}
💵 Капитал: ${capital:,.2f}
📅 День: {user['day']}
🏆 Кубков: {user.get('cubes', 0)}
📦 Предметов: {len(user.get('inventory', []))}
⏱️ Последний вход: {user.get('last_seen', 'никогда')}
...
📋 *Портфель:*
        ...
        if user.get('portfolio'):
            ...
            for ticker, amount in user['portfolio'].items():
                ...
                text += f"\n{ticker}: {amount} шт. (${amount * price:,.2f})"
        else:
            text += "\nПусто"
...
@bot.message_handler(commands=['adminuserlist'])
def admin_user_list(message):
    ...
    users.sort(key=lambda x: x['capital'], reverse=True)
...
@bot.message_handler(commands=['adminadditem'])
def admin_add_item(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) < 3:
            bot.reply_to(message, "❌ Использование: /adminadditem ID ITEM_ID")
            return
        ...
        user['inventory'].append({
            ...
            'emoji': SHOP_ITEMS[item_id]['emoji'],
            'description': SHOP_ITEMS[item_id]['description'],
            'effect': SHOP_ITEMS[item_id].get('effect', ''),
            'bought_at': str(datetime.now())
        })
        
        save_user(target_id, user)
        bot.reply_to(message, f"✅ Предмет {SHOP_ITEMS[item_id]['emoji']} {SHOP_ITEMS[item_id]['name']} выдан пользователю {target_id}")
    except ValueError:
        bot.reply_to(message, "❌ ID должен быть числом")
    except Exception as e:
        bot.reply_to(message, f"❌ Ошибка: {e}")

@bot.message_handler(commands=['adminremoveitem'])
def admin_remove_item(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) < 3:
            bot.reply_to(message, "❌ Использование: /adminremoveitem ID ITEM_ID")
            return
        ...
        if 'inventory' not in user or not user['inventory']:
            bot.reply_to(message, "❌ У пользователя нет предметов")
            return
        ...
        if removed:
            save_user(target_id, user)
            bot.reply_to(message, f"✅ Предмет {removed_item['emoji']} {removed_item['name']} удалён у пользователя {target_id}")
        else:
            bot.reply_to(message, f"❌ У пользователя нет предмета {item_id}")
    except ValueError:
        bot.reply_to(message, "❌ ID должен быть числом")
    except Exception as e:
        bot.reply_to(message, f"❌ Ошибка: {e}")
...
@bot.message_handler(commands=['adminbroadcast'])
def admin_broadcast(message):
    if message.from_user.id != ADMIN_ID:
        return
    
    try:
        text = message.text.replace('/adminbroadcast', '').strip()
...
@bot.message_handler(commands=['adminbroadcastactive'])
def admin_broadcast_active(message):
    if message.from_user.id != ADMIN_ID:
        return
    
    try:
        text = message.text.replace('/adminbroadcastactive', '').strip()
...
if __name__ == "__main__":
    print("🤖 Terminal Trade (ФИНАЛЬНАЯ ВЕРСИЯ) запущен...")
