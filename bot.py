...
def inventory_cmd(m):
    ...
    try:
        ...
        print(f"Error in inventory_cmd: {e}")
        bot.send_message(uid, text, reply_markup=inventory_kb(user))
...
def callback_handler(call):
    ...
    print(f"[DEBUG] Callback received: {data}")  # Отладка
    ...
    try:
        ...
        if data == "next":
            ...
            text = "⏩ Новый день! Цены обновлены."
            
            if call.message.photo:
                bot.delete_message(call.message.chat.id, call.message.message_id)
                bot.send_message(call.message.chat.id, text, reply_markup=back_kb("back_to_main"))
            else:
                bot.edit_message_text(
                    text,
                    call.message.chat.id,
        ...
        if data.startswith("shop_item_"):
            item_id = data[10:]  # Убираем "shop_item_"
            item = SHOP_ITEMS.get(item_id)
        ...
        if data.startswith("buy_item_"):
            item_id = data[9:]  # Убираем "buy_item_"
        ...
        if data.startswith("inv_item_"):
            idx = int(data[9:])  # Убираем "inv_item_"
            user = get_user(uid)
        ...
        if data.startswith("use_item_"):
            idx = int(data[9:])  # Убираем "use_item_"
        ...
        # ===== ПОМОЩЬ =====
        if data == "help":
            help_text = "Используй кнопки главного меню для навигации.\n\n"
            help_text += "📊 Статус - твоя статистика\n"
            help_text += "📈 Компании - список компаний и цены\n"
            help_text += "⏩ Next - следующий день (цены меняются)\n"
            help_text += "🏆 Лидеры - топ игроков\n"
            help_text += "🛒 Магазин - покупка предметов\n"
            help_text += "🎒 Инвентарь - твои предметы\n"
            help_text += "🎟️ Промо - список промокодов\n"
        ...
        if data.startswith("confirm_promo_"):
            code = data[14:]  # Убираем "confirm_promo_"
        ...
        print(f"[WARNING] Unknown callback: {data}")
...
if __name__ == "__main__":
    ...
    print("🔥 Отладка включена - смотрите консоль для callback'ов")
    
    # Добавим функцию delete_promocode, если её нет
    if 'delete_promocode' not in globals():
        def delete_promocode(code):
            p = load_promocodes()
            if code in p:
                del p[code]
