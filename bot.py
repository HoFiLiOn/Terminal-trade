...
IMAGES = {
    ...
    'promo': 'https://s10.iimage.su/s/05/gIKluIYxLgUpL28XWRh1IzTmdY4DzqMQuX8v39ee9.jpg'  # Для промо используем главное фото
}
...
def promo_activate_cmd(m):
    try:
        ...
        if promo:
            ...
            if IMAGES.get('promo'):
                bot.send_photo(m.chat.id, IMAGES['promo'], caption=text, reply_markup=markup)
            else:
...
def promo_list_cmd(m):
    ...
    if IMAGES.get('promo'):
        bot.send_photo(m.chat.id, IMAGES['promo'], caption=text, reply_markup=back_kb("back_to_main"))
    else:
        bot.send_message(m.chat.id, text, reply_markup=back_kb("back_to_main"))
...
def callback_handler(call):
    ...
    try:
        ...
        if data == "leaderboard":
            ...
            if IMAGES.get('leaderboard'):
                bot.send_photo(uid, IMAGES['leaderboard'], caption=text, reply_markup=back_kb("back_to_main"))
            else:
        ...
        if data == "shop":
            ...
            if IMAGES.get('shop'):
                bot.send_photo(uid, IMAGES['shop'], caption=text, reply_markup=shop_kb())
            else:
                bot.send_message(uid, text, reply_markup=shop_kb())
        ...
        if data.startswith("shop_category_"):
            ...
            bot.send_message(uid, text, reply_markup=shop_category_kb(category))
        ...
        if data == "promo":
            ...
            if IMAGES.get('promo'):
                bot.send_photo(uid, IMAGES['promo'], caption=text, reply_markup=back_kb("back_to_main"))
            else:
                bot.send_message(uid, text, reply_markup=back_kb("back_to_main"))
        ...
        if data == "help":
            ...
            bot.send_message(uid, help_text, parse_mode="Markdown", reply_markup=back_kb("back_to_main"))
        ...
        if data.startswith("page_"):
            ...
            if IMAGES.get('companies'):
                bot.send_photo(uid, IMAGES['companies'], caption=text, reply_markup=companies_kb(page, pages))
            else:
                bot.send_message(uid, text, reply_markup=companies_kb(page, pages))
...
if __name__ == "__main__":
    ...
    print("✅ Разные фото для разных разделов")
    print("✅ У промокодов есть фото")
    print("✅ Один текст - предыстория")
    print("✅ Добавлена команда /addleader для админа")
