...
def get_shop_keyboard():
    ...
    for item_id, item in SHOP_ITEMS.items():
        if item['price'] > 0:  # Не показываем кубки в магазине
            markup.add(
...
if __name__ == "__main__":
    ...
    print("/admin_give ID СУММА — выдать деньги")
    print("/admin_take ID СУММА — забрать деньги")
    print("/admin_season_create НАЗВАНИЕ ДНЕЙ — создать сезон")
    print("/admin_season_end — завершить сезон")
