...
# ========== ТОКЕН ==========
TOKEN = "8720385396:AAFy-3FIdjUMDycHaVrSnwdsXAnMMsC1GHw"
...
DEFAULT_COMPANIES = {
    ...
    'NFLX': {'name': 'Netflix Inc.', 'price': 450.30, 'prev_price': 450.30, 'volatility': 0.12},
    'DIS': {'name': 'Walt Disney Co.', 'price': 110.20, 'prev_price': 110.20, 'volatility': 0.09},
    'PYPL': {'name': 'PayPal Holdings', 'price': 85.40, 'prev_price': 85.40, 'volatility': 0.11},
    'ADBE': {'name': 'Adobe Inc.', 'price': 520.10, 'prev_price': 520.10, 'volatility': 0.1},
    'INTC': {'name': 'Intel Corp.', 'price': 32.80, 'prev_price': 32.80, 'volatility': 0.08},
    'AMD': {'name': 'AMD Inc.', 'price': 140.60, 'prev_price': 140.60, 'volatility': 0.13}
}
...
def set_company_price(ticker, price):
    ...
    old_price = companies[ticker]['price']
    companies[ticker]['prev_price'] = old_price
...
def force_update_leaderboard():
    """Принудительно обновляет таблицу лидеров"""
    leaderboard = []
    for filename in os.listdir(USERS_DIR):
        if filename.endswith('.json'):
            user_id = filename.replace('.json', '')
            user = get_user(int(user_id))
            if user and user.get('banned', 0) == 0:
                capital = calculate_capital(int(user_id))
...
def check_season_end():
    ...
    if days_left is None:
        # Вайп всех данных при завершении сезона
        for filename in os.listdir(USERS_DIR):
            if filename.endswith('.json'):
        ...
        # Создаём новый сезон
        create_season('НОВЫЙ СЕЗОН', 14)

# Запускаем проверку сезона в отдельном потоке
...
def get_back_keyboard():
    ...
    btn = types.InlineKeyboardButton("◀️ В главное меню", callback_data="back")
    markup.add(btn)
    return markup
...
def get_companies_keyboard(current_page, total_pages):
    ...
    btn_back = types.InlineKeyboardButton("◀️ В главное меню", callback_data="back")
    markup.add(btn_back)
...
# ========== АДМИН-КОМАНДЫ (ПОЛНОЕ УПРАВЛЕНИЕ) ==========
...
/admin info ID — информация об игроке

**📈 УПРАВЛЕНИЕ КОМПАНИЯМИ:**
/admin comps — список всех компаний
/admin comp add ТИКЕР НАЗВАНИЕ ЦЕНА — добавить компанию
/admin comp del ТИКЕР — удалить компанию
/admin comp price ТИКЕР ЦЕНА — изменить цену
/admin comp vol ТИКЕР 0.1 — изменить волатильность

**🎟️ УПРАВЛЕНИЕ ПРОМОКОДАМИ:**
/admin promo add КОД БОНУС МАКС — добавить промокод
/admin promo del КОД — удалить промокод
/admin promos — список всех промокодов

**📅 УПРАВЛЕНИЕ СЕЗОНАМИ:**
/admin season create НАЗВАНИЕ ДНЕЙ — создать сезон
/admin season end — завершить текущий сезон
/admin season — информация о текущем сезоне

**📢 РАССЫЛКИ:**
/admin broadcast ТЕКСТ — отправить всем игрокам
/admin broadcast active ТЕКСТ — отправить активным (за 7 дней)

**📊 СТАТИСТИКА:**
/admin stats — общая статистика
/admin top — топ-10 игроков
    """
...
🆔 ID: {target_id}
👤 Имя: {user.get('username', 'Нет')}
💰 Баланс: ${user['money']:,.2f}
💵 Капитал: ${capital:,.2f}
📅 День: {user['day']}
⛔ Бан: {'Да' if user.get('banned') == 1 else 'Нет'}

📊 Портфель:
        """
        
        portfolio = get_portfolio(target_id)
        if portfolio:
            for p in portfolio:
                text += f"\n{p['ticker']}: {p['amount']} шт. (${p['amount'] * p['price']:,.2f})"
        else:
            text += "\nПусто"
        
        bot.reply_to(message, text, parse_mode="Markdown")
    except:
        bot.reply_to(message, "❌ Использование: /admin info ID")
...
if __name__ == "__main__":
    print("🤖 Terminal Trade (АДМИН-ПАНЕЛЬ) запущен...")
