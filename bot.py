...
SHOP_ITEMS = {
    ...
    'spring_cube': {
        'name': 'Весенний кубок',
        'description': 'Эксклюзив сезона ВЕСНА',
        'price': 2000,
        'emoji': '🏆',
        'effect': 'cube',
        'category': 'seasonal',
...
def activate_item(user_id, item_index):
    ...
    if effect == 'booster':
        ...
        msg = "🏆 Кубок добавлен в коллекцию!"
...
def get_shop_main_keyboard():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🏆 Сезонные", callback_data="shop_category_seasonal"),
...
def get_shop_category_keyboard(category):
    ...
    markup.add(types.InlineKeyboardButton("◀️ Назад в магазин", callback_data="back_to_shop_main"))
    return markup
...
def get_item_action_keyboard(item_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
...
def start_command(message):
    ...
    story = (
        ...
        "В описании было сказано: _«Хочешь стать миллионером? Начнинай прямо щас и покори биржу!»_.\n\n"
...
Используй /help для списка команд
    ...
    elif call.data == "back_to_shop_main":
        text = "🛒 *МАГАЗИН TERMINAL TRADE*\n\nВыбери категорию:"
    ...
    elif call.data.startswith("shop_category_"):
...
✅ Активировать бонус
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            parse_mode="Markdown",
            reply_markup=get_item_action_keyboard(item['item_id'])
        )
        bot.answer_callback_query(call.id)
...
💳 Твой баланс: ${user['money']:,.2f}
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            parse_mode="Markdown",
            reply_markup=get_item_action_keyboard(item_id)
        )
        bot.answer_callback_query(call.id)
