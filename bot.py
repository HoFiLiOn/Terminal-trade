...
def activate_item(user_id, idx):
    ...
    if item['effect'] == 'booster':
        ...
        # Аналитик - прогноз на следующий день
        companies = load_companies()
        predictions = []
...
def start_cmd(message):
    ...
    text = (
        ...
        "Ты начинающий трейдер. Покупай и продавай акции, зарабатывай деньги, становись лучшим!\n\n"
        f"💰 Твой баланс: ${user['money']:,.2f}\n"
        f"🏆 Кубков: {user.get('cubes', 0)}{time_left}"
    )
...
def next_cmd(m):
    ...
    # Отправляем новое сообщение с фото вместо редактирования
    season = get_current_season()
    d, h = get_season_time_left()
    time_left = f"\n📅 До конца сезона: {d}д {h}ч" if d else ""
...
if __name__ == "__main__":
    ...
    print("✅ Все кнопки работают, фото сохраняются")
