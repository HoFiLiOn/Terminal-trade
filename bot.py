...
SHOP_ITEMS = {
    ...
    'predictor': {
        'name': '🔮 Предсказатель',
        'description': 'Видит тренд на 3 дня вперёд',
        'price': 800,
        'emoji': '🔮',
        'effect': 'predictor'
    },
...
def activate_item(user_id, item_index):
    ...
    if effect == 'booster':
        ...
        msg = "💎 VIP статус активирован! +10% к доходу навсегда"
...
def get_item_action_keyboard(item_index):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✅ Активировать", callback_data=f"activate_item_{item_index}"),
...
def promo_list_command(message):
    ...
    bot.send_message(message.chat.id, text, reply_markup=get_back_keyboard("main"))
...
✅ Активировать бонус
        ...
        bot.edit_message_text(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=get_item_action_keyboard(item_index)
        )
        bot.answer_callback_query(call.id)
    
    elif call.data.startswith("activate_item_"):
        item_index = int(call.data.replace("activate_item_", ""))
...
if __name__ == "__main__":
    print("🤖 Terminal Trade (С БОНУСАМИ) запущен...")
